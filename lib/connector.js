  ...
  return {
    ...
    findPeersFor(keyword) {
      const lookupPromise = new Promise((resolve) => {
        this.listenPeerLookup((response) => {
      ...
      dht.lookup(this.generateHash(keyword), (err) => {
        if (err) {
    ...
    async removePeerFor(keywordToRemove) {
      ...
      return this.discardCurrentKeywords(keywordsKnown);
    },

    async discardCurrentKeywords(newKeywords = []) {
